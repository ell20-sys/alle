...
export default function ChatPage() {
  ...
  const role = sessionId === "seeker" ? "seeker" : "helper";

  const ws = useRef<WebSocket | null>(null);

  useEffect(() => {
    ws.current = new WebSocket(`ws://localhost:8080/${role}`);
    ws.current.onmessage = (event) => {
      setMessages((prev) => [...prev, event.data]);
    };

    ws.current.onclose = () => {
      setMessages((prev) => [...prev, "Chat session ended."]);
    };
  ...
  return (
    <div className="flex items-center min-h-screen">
      <Card className="w-full max-w-2xl mx-auto">
        ...
        <CardContent>
          <ScrollArea className="h-[400px] pr-4 border border-gray-500 rounded-lg mb-1">
